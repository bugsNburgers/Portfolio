...
const StyledProjectsGrid = styled.ul<{ $showMore: boolean }>`
  ${({ theme, $showMore }) => css`
    display: grid;
    ...
    li {
      display: ${({ $showMore }: { $showMore: boolean }) => 'block'};
    }
  `}
`;
  ...
  return (
    <StyledOtherProjectsSection ref={ref as React.RefObject<HTMLElement>}>
      ...
      <motion.ul
        ...
        style={{ padding: 0, margin: 0, listStyle: 'none', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '15px' }}
        initial={prefersReducedMotion ? 'visible' : 'hidden'}
        animate={isInView || prefersReducedMotion ? 'visible' : 'hidden'}
